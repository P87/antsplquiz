    ...
    fetch("/questions/player-search", {
      ...
      .then((result) => {
        if (result.success) {
          ...
          setErrorMessage("There was an error searching for your player, please refresh and try again");
        }
      })
  }
  ...
  return (
    <div>
      ...
      <div className="alert alert-warning text-center" role="alert">
        Start typing the name of the player you want and select them from the list.<br />
        <strong>If you can't find the player you want please inform Ant ASAP so he can get them added!</strong>
      </div>
      <div className="form-floating">
        <input type="text" className="form-control" id="player" name="player" placeholder="Player" onChange={handleSearchChange}
               value={searchText} />
        <label htmlFor="username" className="form-label">Player</label>
      </div>
      <div id="players-list">
        <div className="list-group">
        {
          foundPlayers.length !== 0 && foundPlayers.map(
            (player) =>
                <a href="#" className="list-group-item list-group-item-action" onClick={() => handlePlayerSelected(player)}>
                  {player.answer}
                </a>
        )}
        </div>
      </div>
      { answer.length !== 0 && <ul className="nav nav-tabs">
        <li className="nav-item mt-3">
          <a className="nav-link active" aria-current="page" href="#">Your Answer(s)</a>
        </li>
        </ul>
      }
      <ul id="selected-players" className="list-group list-group-flush">
        {
          answer.map(player => <li className="list-group-item">
            <div className="row">
              <div className="col-10 fw-bold">
                {player.answer}
              </div>
              <div className="col-2 text-end">
                <button className="btn btn-sm btn-danger pl-2" onClick={() => removeSelectedPlayer(player)}>Remove</button>
              </div>
            </div>
          </li>)
        }
